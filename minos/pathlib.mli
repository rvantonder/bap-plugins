...
(** [state] is the local state passed to each path. I typically use it
    ...
    TODO: this should be modified to use Graphlib, like fold_paths_prog. *)
val fold_paths: sub:Sub.t -> state:'a -> acc:'b -> blk:tid ->
  finish:('a -> tid -> 'b -> 'b) ->
...
(** Operates in the same manner as fold_paths, but starts at a given
    ...
    in reverse *)
val fold_paths_prog: ?rev:bool -> prog:Graphlib.Callgraph.t -> state:'a ->
  acc:'b -> sub:tid -> finish:('a -> tid -> 'b -> 'b) ->
  f:('a -> tid -> (state:'a -> 'b) -> 'b -> 'b) -> 'b
...
(module Graphlib.Graph with type edge = Graphlib.Tid.Tid.edge and type node =
 tid and type t = Graphlib.Tid.Tid.t) ->
